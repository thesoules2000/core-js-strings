...
function formatTime(minutes, seconds) {
  ...
  const formattedSeconds = seconds.toString(10).padStart(2, '0');
...
function isPalindrome(str) {
  const strFormed = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const strReversed = strFormed.split('').reverse().join('');
  return strFormed === strReversed;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    str.charAt(i).toUpperCase() === str.charAt(i)
...
function extractNameFromTemplate(value) {
  // eslint-disable-next-line no-param-reassign
  value = value.split(', ');
  return value[1].replace(/[^a-zA-Z0-9]/g, ' ').trim();
}
